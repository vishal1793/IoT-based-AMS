...
from app.models import Asset_table
...
def UID(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                asset = Asset_table.objects.get(uid=uid)
            except Asset_table.DoesNotExist:
                print("Asset with UID {} does not exist.".format(uid))
                asset = None

            print("Asset:", asset)  # Debug output

            return render(request, 'confirm.html', {'kitid': kit_id, 'loc': loc_place, 'uid': uid, 'asset': asset})
    else:
        form = UIDForm()
        ...
        if req.method == 'POST':
            ...
            if existing_entry.exists():
                if existing_entry.first().kit_id != kitid:
                    ...
                    return HttpResponse("This is already entered")
            else:
                # Localize time to Indian timezone
                in_loc_ts = timezone.now().astimezone(pytz.timezone('Asia/Kolkata'))
                Kit_table.objects.create(kit_id=kitid, in_loc_place=loc_place, uid=asset_instance, in_loc_ts=in_loc_ts)
        ...
        if req.method == 'POST':
            ...
            if existing_entry.exists():
                if existing_entry.first().kit_id != kitid:
                    ...
                    return HttpResponse("This is already exited")
            else:
                # Localize time to Indian timezone
                out_loc_ts = timezone.now().astimezone(pytz.timezone('Asia/Kolkata'))
                Kit_table2.objects.create(kit_id=kitid, out_loc_place=loc_place, uid=asset_instance, out_loc_ts=out_loc_ts)
